# KeyWords In  CS

> **Please sort alphabetically /  请按字母顺序排序**

## Reference

[Symbols table in Chinese and English/常见符号表中英对照](https://www.gwduan.com/web/computer/languages/c/punctuation.html)

[Math operation in Chinese and English/ 常见数学操作表对照表](https://zhuanlan.zhihu.com/p/136367900?ivk_sa=1024320u)



## Bit Manipulation / symbols

1. binary solution/operation: 位运算
2. ^ : 异或 ：bitwise  XOR
3. ~：取反： bitwise NOT / flip / bitwise negation 
4. & : 并: bitwise AND
5. | : 或: bitwise OR
6. << : 左移 : left Shift 
7. `>>` : 有移： right Shift
8. `-` : hyphen: 连接号
9. `--` : dash : 破折号 （比hyphen长）
10. `！`： exclamation mark ： 感叹号
11. `,` : comma: 逗号
    1. Using a comma as a delimiter ： 用逗号作为分隔符 

12. `？`： question mark
13. mop this bit up :  擦去这个 比特位
    1. erase this bit place up: 抹去这个 比特位 
14. 二进制： binary
    1. 10进制：Decimal
    2. 16进制 ： Hexadecimal  
    3. 8进制： octal


## C++/Python/Java Specific Vocabulary

1. `&` ：references ： 引用
2. `*`：dereference or star or pointer ：指针 指向， 
2. `-`：delimiter ： 分界符 
2. Reflection Mechanism: 反射机制

##  

## Complexity Analysis 

1.  O(n) :  big O of N; `Linear time = Single pass = Single iteration`
2.  O(N^2): big O of N square 
3.  O(N^3) : big O of N cube
4.  O(1) : Only constant space/time required /  uses only constant extra space. 
5.   $ \log_{10}(x) $ : log base ten of x: 以10为底x的对数
4.  Time Limit Exceeded: 超时！
7.  `O(3^N)`: big O of three to the Nth power 
    1.  或者 big O of Nth power of three
8.  `O(n!)`: big O of  factorial of n 
8.  `O(sqrt(N))`:  big O of squart root of n

## Describing  Algorithm

1. abs() :  absolute value of 绝对值：

2. allocate : 分配

   1. allocated  memory： 分配内存
   2. allocated array: 分配的数组 

3. alphanumeric ： 字母数字

4. Ascending Sort : 升序排序

   1. Descending Sort： 降序排序

5. Appear n times: 出现了n次 

   1. showed n times ：展现了n次 
   2. visited n times： 访问n次
   3. counting the times of appearance : 记录出现的次数
   3. occur six times: 出现了6次

6. boilerplate code: 模板代码

7. brackets：括号

   1. parentheses ： 括弧
   1. nesting parentheses： 嵌套括号
   1. open/left parentheses:  `(`
   1. close/right parentheses: `)`

8. backwards: 向后

   1. right pointer  goes backwards:  右指针反向走
   2. left pointers goes forwards:  左指针正向走

9. Bit Manipulation ：位运算/位操作

10. B is going be set to A： 把A赋值给B

   11. copy the value from A to B： 把A赋值给B

12. Bounds/boundary

   13. out of bounds: 越界

   14. in bounds: 在界限内

15. Commented out : 注释掉

16. Convergence： 收敛

17. Declare the variable:  声明变量

    1. initialize the variable : 初始化变量

18. Diagonal  : 对角线

    - anti-diagonal：反对`角线`
    - move diagonally:  对角线移动

19. **distinct** element: 独立元素，不同元素

    1. the `kth` **distinct** element： 第K个不同的元素

20. Disproportionate choice : 不成比例的选择，不随机了

    1. Proportionate / Random choice: 随机的选择，成比例的选择

21. De-duplication:  去重

22. Declare the variable: 声明变量

23. endless loops ：死循环

24. etc ； 等等

25. indentation : 缩进

26. If statements: if 语句(声明)， problem statements: 问题描述
    1. the conditions of the first If statements / block：第一个if模块的条件
    1. **the else case = opposite case**  ，对else的情况/相反的情况
    1. we will do similary in the opposite case :  else的情况也是类似的做法

27. implement ：填充，实现

    1. Populating ：填充

28. initialize :  初始化

29. instantiated : 实例化

30. inner loop: 两层循环

31. infinite loop: 死循环

32. increment ：递增，decrement ： 递减；  

    1. pointer left  increment by one (step): 左指针加一
    1. right pointer decrementing : 右指针递减

33. interval: 区间

34. Kind of  modified  A： A的变种版

35. loop : 循环

36. loop stop executing: 循环停止

37. moves one spot of head: 链表从头指针走一步

38. moves the pointers inwards: 让两头指针向内走，让两头指针聚拢

39. Negate ： 取反

    1. flip 

40. non-null /nang nəl/ :非空

41. Not simple and elegant enough：不够简洁优雅

    1. Not the prettiest way to XX: 不是最优雅的方式

42. odd： 奇数， even：偶数

43. one's places value： 每一位的值；eg：get one's places value `1， 2，3`  from  `123` 

44. optimal approach/solution : 最佳方法、方案

45. pass N to function:  给函数传参 N

46. permutation：排列

    1. combination: 组合

47. Pop them off： 弹出他们

48. Present : 存在/出现

    1. if item not present: 如果元素没有出现

49. priority：优先级 ；A has higher priority

50. precedence： 优先级； A has the highest precedence : A有最高有优先级

51. prefix sum: 前缀和

    1. suffix sum: 后缀和

52. remainder： 余数； 除法(fraction divide)的余数， 

    1. remainder list :   剩余的列表

53. Regular Expression: 正则表达式

54. Rounded up : 向上取整

    1. Round down:  向下取整
    1. round towards zero: 向0舍入

55. shift every time : 每次更新

56. step forward : 前进一步

57. split the array in tow parts:  将数组分成两部分

    1. chop off the integer into different digits: 将整数分割成不同的单数
    2. chop it off:  砍掉它
    3. chop off the last digit of integer:  砍掉整数的最后一位数字 

58. subtract: 减去，代表完整的行为， minus 是 "-"这个symbol

59. sutiable : 非常合适

    1. This formula is very suitable to solve that problem：这个公式非常适合解决这个问题

60. Sum of all unit: 所有单元和 

61. Symmetric :  对称的

62. traverse the array : 遍历数组

    1. Scan through the array: 遍历数组
    2. Loop through the array/digit: 遍历数组/数字
    3. **Spiral Traversing**:  螺旋遍历
    4. recurrence: 复发，循环

63. Temporary variable： 临时变量

64. typos: 错别字：

    1. Don't forget your typos: 别忘记处理你的错别字

65. Visit each member/element : 访问每个成员

    1. Scan each element: 扫描每个成员
    1. Go through element/digit by element/digit: 扫描每个成员/数字
    1. Access each member/element : 访问每个成员

    


## Describing  Project

1. deployed : 部署
1. instance: 实例







## Data Structure

1. Array : 数组

   1. expand Array / extend Array : 扩充数组
   1. Product Subarray ： 成绩子数组

2. Binary search: 二分查找

3. bit operation:   位运算

4. Divide and  conquer approach :  分治法

5. Enumeration methods: 枚举法

6. Heap: 堆

   1. **Max-Heap** : 堆顶元素最大
   2. **Min-Heap**: 堆顶元素最小

7. Graph / Topology : 图 拓扑

   1. Transpose graph : 有向图

8. Linked List :链表

   1. *implicit* **LinkedList**.： 隐式链表
   2. Singly linked list ： 单链表
   3. Doubly linked list : 双向链表

9. List : 列表（一般指数组 Array）

   1. List of List: 二维数组
      1. two dimension list/array: 

      2. nested list

   2. Adjacent List: 临近/界数组 

10. LRU : least recently used 最近最少使用

   1. evict the least recently used key : 驱逐最近最少使用的 key

11. Stack : 栈

    1. monotonic stack: 单调栈

12. Tree

    1. binary tree ： 二叉树
    2. ternary tree：三叉树
    3. left subTree: 左子树
    4. BInary Tree: 二叉树
       1. Binary Search Tree: 二叉搜索树
    5. Inorder Traversal: 中序遍历
       1. preOrder Traversal : 先序遍历
       2. postOrder Traversal: 后序遍历
          1. subsequent Traversal ：  后序遍历
       3. Level Order Traversal ： 层级遍历
       4. Zigzag Level Order Traversal: z字形顺序遍历
    6. descendants ： 子孙节点
    7. Ancestor: 祖先节点

13. Priority  Queue： 优先级队列

    

    


## Math  Related

1. Arithmetic expression： 算数表达式

1. Arithmetic Progression: 等差数列

3. Multiplication ： 乘法 

   1. multiple / times : 乘 
   2. Product ：乘

4. Division : 除法

   1. Divide : 除
   2. Remainder : 余数
   3. truncate toward zero : 向0截断

5. Factorial : 阶乘

   1. Factorial  Trailing Zeroes： 阶乘尾随的0

6. Fraction  ： 分数

   1. Denominator  : 分母， 除数
   2. Numerator  ： 分子，被除数
   3. Remainder : 余数

7. **Linear algebra** : 线性代数

   - Transpose：（矩阵）转置

   - Reflect ： 镜像（反射）

8. Prime number : 质数

   1. Composite number： 合数
   2. factor ： 因数，因子

9. Reciprocal: 倒数

10. Recurring Decimal： 循环小数

11. Reverse Polish Notation: 逆波兰等式

12. Square root : 平方根 

    



## Specialism  CS Details

1. decimal ： 十进制

2. instance: 实例

3. Object-oriented design： 面向对象设计

4. Web Scraper： 网页爬虫.

   
